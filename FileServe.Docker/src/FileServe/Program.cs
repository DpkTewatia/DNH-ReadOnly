...
// Relevant when a reverse proxy (nginx, Traefik, an ingress controller) sits in front of
// the container. The default KnownProxies list is loopback-only, so these headers are
// ignored unless the hop is local -- see README for what to add when it is not.
...
{
    // In a container this is nearly always a missing volume mount, or a host folder the
    // container's non-root user cannot read. Fail at startup rather than 404 every request.
    logger.LogCritical(
        "FileVault:RootPath '{RootPath}' does not exist inside the container, or is not readable " +
        "by uid {Uid}. Check the volume mount and the host folder's permissions.",
        rootPath,
...
// Serving the application's own directory would expose appsettings.json and the compiled
// assemblies. Refuse to start rather than serve the site to itself.
...
app.UseStaticFiles(new StaticFileOptions
{
    ...
    OnPrepareResponse = context =>
    {
        ...
        // The vault is an allowlist of images and text, so a browser sniffing its way to
        // some other type is never something we asked for.
